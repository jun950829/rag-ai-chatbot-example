...
export function ChatbotPage() {
  ...
  const runRagPipelineFixed = async (text: string) => {
    setStatusLine('요청을 큐에 등록하는 중...')
    const botId = newId()
    appendMessage({ id: botId, role: 'bot', text: '', typing: true, cards: undefined })
    let accumulated = ''
    let cardsAccumulator: SuggestionCard[] | undefined
    try {
      const { request_id: rid } = await postChatQueue(text.trim(), sessionId)
      setStatusLine('응답을 기다리는 중...')
      await streamChatAnswer(rid, {
        onToken: (displayed) => {
  ...
  const onCarouselDetail = (card: SuggestionCard) => {
    const follow =
      String(card.follow_prompt || '').trim() ||
      `${String(card.title || '').trim()}에 대해 자세히 알려줘`
    handleSubmitComposer(follow)
  }
