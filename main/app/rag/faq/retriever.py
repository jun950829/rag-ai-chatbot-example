...
class FaqRetriever:
    ...
    def search(
        ...
        *,
        ...
        # raw → normalized → canonical 을 거쳐 최종 검색 쿼리를 만든다.
        # (canonicalize는 `normalize_faq_query` 내부에서 처리됨)
        ...
        # qa_user 필터: visitor/exhibitor 모드에서는 해당 user + (NULL/빈값) 허용
        user_filter_sql = ""
        # 유저 입력 원문과 canonical 모두를 파라미터로 전달
        ...
        # 1) alias exact match (normalized_question == query_norm)
        ...
        # 2) FTS/Trgm 후보 풀 (FTS 20 + TRGM 20) → merge/dedupe 후 rerank
        sql = text(
            f"""
            WITH fts AS (
              SELECT
                ...
                q.links,
                q.notes,
                -- 가중 tsvector(있으면 사용). 없으면 on-the-fly로 계산.
                ts_rank(
                  COALESCE(
                    q.faq_search_tsv,
                    (
        ...
        try:
            ...
            # alias 테이블/pg_trgm 확장이 아직 없을 수 있다.
            # 1차 실패 시에는 FTS 전용(확장 불필요) 쿼리로 한 번 더 시도한다.
        ...
        # exact + (fts/trgm) 합산 후보풀
        trace["fts_trgm_candidates"] = len(rows)
        cands: list[FaqCandidate] = []
        # dedupe by qna_code (keep best raw signals)
        ...
        for r in by_code.values():
            ...
            fts_rank = float(r.get("fts_rank") or 0.0)
            ...
            # category boost: query에 category/subcategory/domain이 포함되는 경우에만 아주 작은 가산점
