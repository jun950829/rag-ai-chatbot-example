...
SSE 이벤트 계약: ``app.streaming.sse_events`` (``stage``, ``retrieval``, ``citation``, ``delta``, ``final``, ``error``, ``done``).
"""
...
from app.cache.redis_keys import chat_answer_cache_key, llm_stream_key, llm_trace_key
from app.core.config import get_settings
from app.streaming.redis_chat_sse import iter_redis_stream_as_sse
...
async def enqueue_chat(
    ...
    stream_key = llm_stream_key(stream_prefix=settings.llm_stream_prefix, request_id=request_id)
    trace_key = llm_trace_key(trace_prefix=settings.llm_trace_prefix, request_id=request_id)
    cache_key = chat_answer_cache_key(prefix=settings.chat_cache_prefix, message=message)
    ...
    try:
        ...
        if cached:
            await redis.rpush(stream_key, stream_payload_json(EVENT_FINAL, cached))
...
async def stream_chat(request_id: str) -> StreamingResponse:
    ...
    async def event_generator() -> AsyncIterator[str]:
        try:
            async for chunk in iter_redis_stream_as_sse(redis, stream_key=stream_key):
                yield chunk
        finally:
            await redis.close()
