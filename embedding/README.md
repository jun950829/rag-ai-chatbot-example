...
벡터 검색·의도 분류·UI는 **`app/` (Docker에서 실행되는 API)** 의 `app/rag/` 와 `/tools/embedding` 에 있습니다.
...
| `app/rag/pipeline.py` | DB 읽기/쓰기, 배치 임베딩, pgvector 검색 SQL (API 프로세스에서 사용) |
| `app/rag/retrieval.py` | 의도·쿼리 계획·RRF 등 검색 파이프라인 |
| `app/rag/search_service.py` | 검색 + OpenAI 답변 조립 |
| `app/templates/embedding_tool.html` | 임베딩/검색 UI (브라우저는 API 서버에만 붙음) |

로컬 워커는 `app.rag.pipeline` 을 import 하므로, **프로젝트 루트를 `PYTHONPATH`에 두고** 실행해야 합니다.
...
3. 브라우저: `http://localhost:8000/tools/embedding`  
   ...
   - 검색은 API 프로세스의 `app/rag` 가 DB 조회 후, 질문 벡터만 로컬 `POST /v1/embed/query` 로 요청
