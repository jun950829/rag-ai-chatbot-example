...
- `main/app/db/repositories/qa_quickmenu_repository.py`
- `scripts/load_qa_quickmenu.py` — CSV → DB 적재
...
## 5. QA 카테고리 퀵메뉴 (`qa_quickmenu`)

CSV (`docs/QA bot_초안.csv`) → Alembic 테이블 → `scripts/load_qa_quickmenu.py` 적재.  
Docker: `docker compose exec api python scripts/load_qa_quickmenu.py` (앱 이미지 **재빌드** 후 신규 모듈 반영).
...
- `main/app/db/repositories/qa_quickmenu_repository.py`
- `main/app/db/models/qa_quickmenu.py`
